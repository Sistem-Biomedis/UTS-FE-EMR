...
const AddDokter = () => {
  ...
  const [formData, setFormData] = useState({ name: '', specialty: '', schedule: '' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-8">
      ...
      <form onSubmit={editingId ? handleUpdateDokter : handleAddDokter} className="mb-6">
        <input
          type="text"
          name="name"
          placeholder="Name"
      ...
      <ul>
        {dokters.map((dokter) => (
          <li key={dokter.id} className="mb-4 p-4 border border-gray-300 rounded">
            <div>
            ...
            <div className="mt-2">
              <button onClick={() => handleEditDokter(dokter)} className="bg-yellow-500 text-white px-2 py-1 mr-2 rounded">
                Edit
              </button>
              <button onClick={() => handleDeleteDokter(dokter.id)} className="bg-red-500 text-white px-2 py-1 rounded">
                Hapus
              </button>
            </div>
          </li>
